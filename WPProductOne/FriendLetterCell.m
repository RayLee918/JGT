...
@implementation FriendLetterCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.headPhoto = [[UIImageView alloc] initWithFrame:CGRectMake(15, 5, 34, 34)];
        [self.contentView addSubview:self.headPhoto];
        
        self.tagLabel = [UILabel new];
        self.tagLabel.frame = CGRectMake(15 + 34 + 5, CGRectGetMinY(self.headPhoto.frame), 30, 15);
        ...
        self.msgImageView = [[UIImageView alloc] initWithFrame:CGRectMake(30 + 34, 25, kScreentWidth - 60 - 68, 40)];
        self.msgImageView.image = kImageNamed(@"msg_background_white.png");
        [self.contentView addSubview:self.msgImageView];
        
        self.msgLbl = [[UILabel alloc] initWithFrame:CGRectMake(20, 10, self.msgImageView.frame.size.width - 35, self.msgImageView.frame.size.height - 20)];
...
- (void)layoutSubviews {
    ...
    self.msgImageView.frame = CGRectMake(30 + 34, 25, size.width + 35, size.height + 20);
    UIImage* img=[UIImage imageNamed:@"msg_background_white.png"];//原图
    UIEdgeInsets edge=UIEdgeInsetsMake(10, 20, 10, 15);
    img = [img resizableImageWithCapInsets:edge resizingMode:UIImageResizingModeStretch];
    self.msgImageView.image=img;
    
    self.msgLbl.frame = CGRectMake(20, 10, size.width, size.height);
}
